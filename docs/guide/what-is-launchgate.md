...
Launchgate is a **self-hosted, open-source feature flag engine** built for teams that need fast, reliable flag evaluation without sending data to a third-party SaaS vendor.
...
## The Launchgate Approach

Launchgate takes a different approach: **local evaluation**.

1. Your server runs the Launchgate control plane (a single Rust binary)
...
| Feature | Launchgate | LaunchDarkly / Statsig |
|---|---|---|
...
## What Launchgate Is Not

- **Not a A/B testing platform** — Launchgate focuses on feature flags and rollouts, not experiment analysis
- **Not a managed service** — you operate the server yourself
- **Not a data warehouse** — Launchgate does not store analytics or events (that is your job)
