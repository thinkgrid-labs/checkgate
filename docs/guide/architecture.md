...
Launchgate has three main layers: the **control plane** (server), the **evaluation core** (Rust library), and the **SDK clients**.
...
3. It publishes a `UPSERT` event to the `launchgate_updates` Redis channel
4. All connected SSE clients (SDK instances) receive the event and update their local cache

### Multi-Instance Deployments

When running multiple Launchgate server instances behind a load balancer:
...
## Evaluation Core (`launchgate-core`)
